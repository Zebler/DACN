...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("TEST COMPONENT 4: TIME PARSER")
    print("=" * 70)
    print(f"Current time: {parser.current_time.strftime('%Y-%m-%d %H:%M:%S')}\n")
    
    for i, test in enumerate(test_cases, 1):
        print(f"[Test {i}] {test['name']}")
        print(f"Input: {test['components']}")
        result = parser.parse(test['components'])
